        ...
        if($userlevel==1 or $userlevel==3 or $userlevel==5){
            ...
            if($userlevel==3 or $userlevel==5){
                ...
                if($keyword !="" && $stdate !="" && $endate !=""){
                    $sql.=" AND (risk_id='$keyword' OR pro_risk.pro_risk_name LIKE '%$keyword%' or `profile`.`name` LIKE '%$keyword%') AND (date_stamp BETWEEN '$stdate' AND '$endate')";
                }elseif($keyword !=""){
                    $sql.=" AND (risk_id='$keyword' OR pro_risk.pro_risk_name LIKE '%$keyword%' or `profile`.`name` LIKE '%$keyword%')";
                ...
                <div class="panel-heading"><h4><i class="glyphicon glyphicon-list-alt"></i> รายการความเสี่ยงบุคคลในหน่วยงาน </h4></div>
                <div class="panel-body">    
                    <form action="depreport.php" method="get" name="search_form" >
